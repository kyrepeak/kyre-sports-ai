"""MLB Daily Game Picks V2.1.2.3 — reload-safe live-risk badge hotfix.

Keeps V2.1.2 intact while correcting pregame check semantics and forcing a fresh
module import so Streamlit cannot reuse the older in-memory risk function:
...
VERSION = "MLB Daily Game Picks V2.1.2.3 • RELOAD-SAFE LIVE-RISK BADGES"
...
def _risk_context(c, games_df, snap, ts, baseline):
    ...
    # Headline status represents check completeness, not just time-sensitive urgency.
    if lineup_cls != "safe":
...
# V2.1.2 render functions resolve these globals at runtime.
...
def render_daily_game_picks(games_df, section_header=None, status_info=None, team_logo=None, h=None):
    previous._lineup_context = _lineup_context
    previous._risk_context = _risk_context
    st.caption("✅ V2.1.2.3 reload-safe risk check: pending lineup/weather = MONITOR; only confirmed checks = OK; critical changes = REFRESH NOW.")
