...
Additive presentation layer over frozen Game Totals V4. V4 keeps the verified
slate, live FanDuel market, offense-vs-defense, and pace/possession. V5 adds
source-proven ESPN 20+ yard rushing/receiving explosive volume per game beneath
the same matchup card.
...
def _explosive_html(context: dict[str, Any] | None) -> str:
    ...
    for side in ("away", "home"):
        ...
        sides.append(
            ...
            '<div class="kgt-boom-stat"><span>20+ RUSH/G</span>'
            f'<b>{escape(_fmt(row.get("rushing_big_plays_per_game")))}</b></div>'
            '<div class="kgt-boom-stat"><span>20+ RECEIVE/G</span>'
            f'<b>{escape(_fmt(row.get("receiving_big_plays_per_game")))}</b></div>'
            '<div class="kgt-boom-stat"><span>EXPLOSIVE PLAYS/G</span>'
            f'<b>{escape(_fmt(row.get("explosive_plays_per_game")))}</b></div>'
            '</div></div>'
        )
    ...
    return (
        '<div class="kgt-boom">'
        '<div class="kgt-boom-head"><b>💥 EXPLOSIVE SCORING • STEP 5</b><span>HIGH • BALANCED • LOW</span></div>'
        f'<div class="kgt-boom-grid">{"".join(sides)}</div>'
        '<div class="kgt-boom-summary">'
        f'<span>Matchup explosive volume • avg team explosive plays/game {_fmt(matchup.get("average_explosive_plays_per_game"))}</span>'
        f'<b>{escape(matchup_signal)}</b>'
        '</div>'
        '<div class="kgt-boom-note">ESPN 20+ yard rushing + receiving big plays divided by ESPN Games Played • descriptive only • no FanDuel value enters this signal.</div>'
        '</div>'
    )
...
def _progress_html() -> str:
    ...
    return (
        ...
        '<div class="kgt-track"><div class="kgt-fill step5"></div></div>'
...
def _render_hero() -> None:
    st.markdown(
        ...
        + '<div class="kgt-sub">Step 5 adds ESPN 20+ yard rushing and receiving explosive volume per game beneath pace and possession. Projection remains OFF and sportsbook influence stays 0.0%.</div>'
    ...
    st.markdown(
        ...
        '<div class="kgt-tool live"><div class="icon">💥</div><b>Explosive Scoring</b><span>20+ yard rushing and receiving volume per game is live.</span></div>'
        '</div>',
        unsafe_allow_html=True,
    )
    st.markdown(_progress_html(), unsafe_allow_html=True)
    st.markdown(
        '<div class="kgt-banner"><strong>Step 5 firewall:</strong> explosive-play volume is descriptive only. '
...
def _cached_explosive_profiles(day_str: str, team_rows: tuple[tuple[str, str], ...]) -> dict[str, dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=min(8, len(unique))) as pool:
        futures = {pool.submit(build_team_explosive_profile, abbr, name, day_str): abbr for abbr, name in unique}
...
def render_nfl_game_totals_hub() -> None:
    ...
    default_date = st.session_state.get(
        ...
        st.session_state.get(
            ...
            st.session_state.get("nfl_game_totals_v3_date", st.session_state.get("nfl_v1_date", pd.Timestamp.now(tz=v4.v3.v2.foundation.ET).date())),
        ),
    )
    ...
    st.caption(
        f"{MODEL_VERSION} • sportsbook projection influence {SPORTSBOOK_PROJECTION_INFLUENCE:.1f}% • projection OFF • live market ON • offense/defense ON • pace/possession ON • explosive scoring ON • wager actions OFF"
    )
