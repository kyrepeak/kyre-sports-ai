"""Step 8 final 14-game NFL player-prop production-readiness proof.

Locked definition of done:
- September 20, 2026 resolves to exactly 14 games / 28 playing teams;
- every matchup carries exact ESPN event + team identity;
- every displayed Passing/Rushing/Receiving prop row carries exact ESPN
  event, team and athlete IDs and belongs to the frozen current roster/depth/
  game-day eligible pool;
- Receptions shares the current WR/TE/RB/FB identity pool and remains
  fail-closed until its independent surface is certified;
- PENDING/CLOSED/UNVERIFIED games expose zero eligible display candidates;
- frozen Step 6 market gates and Step 7 app render gates are present;
- no projection, probability, price, grade, rank, stake, wager, or UI behavior
  is modified by this certification.

This file is proof-only. Steps 1-7 remain frozen.
"""
...
def _verify_frozen_step6_step7_chain() -> None:
    """Prove the final audit is sitting on the certified Step 6 + Step 7 owners."""
    from pathlib import Path

    root = Path(__file__).resolve().parents[1]
    app = (root / "app.py").read_text(encoding="utf-8")
    router = (root / "streamlit_memory_lazy_router_v187.py").read_text(encoding="utf-8")
    guard = (root / "nfl_prop_app_eligibility_v1.py").read_text(encoding="utf-8")
    passing = (root / "sports_api/collectors/nfl_passing_yards_render_espn_v2.py").read_text(encoding="utf-8")
    rushing = (root / "sports_api/collectors/nfl_fanduel_rushing_yards_v2.py").read_text(encoding="utf-8")
    receiving = (root / "sports_api/collectors/nfl_fanduel_receiving_yards_v1.py").read_text(encoding="utf-8")
    ...
    for text in (passing, rushing, receiving):
        assert "nfl_prop_market_eligibility_v1 as market_elig" in text
        assert "market_elig.build_event_market_eligibility(" in text
        assert "market_elig.market_row_eligible(" in text


def main() -> int:
    _verify_frozen_step6_step7_chain()
    ...
    report: dict[str, Any] = {
        ...
        "frozen_step6_step7_chain_verified": True,
        "event_audits": [],
    }
    ...
    for event in events:
        ...
        if state == "CONFIRMED":
            ...
            with ThreadPoolExecutor(max_workers=3, thread_name_prefix="step8-market") as pool:
                futures = {
    ...
    print("NFL_STEP8_FULL_14_GAME_PROOF_GREEN")
    return 0
