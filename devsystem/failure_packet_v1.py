...
def render_markdown(packet: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- **History coverage confidence:** {history.get('history_coverage_confidence', 'unavailable')}",
        f"- **History oldest verified prior packet (UTC):** {history.get('history_oldest_verified_at') or 'unavailable'}",
        f"- **History newest verified prior packet (UTC):** {history.get('history_newest_verified_at') or 'unavailable'}",
        f"- **Bounded history lookback:** {history.get('history_lookback') or 'unavailable'}",
        f"- **Bounded history lookback (seconds):** {history.get('history_lookback_seconds') if history.get('history_lookback_seconds') is not None else 'unavailable'}",
        "",
        "## Lane Results",
        "",
        "| Lane | Result |",
