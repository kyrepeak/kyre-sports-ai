"""Kyre Sports API client for NFL Rushing Yards Step 4 market context.

This client is deliberately post-projection. It accepts only a fresh exact-ID
Rushing Yards market payload and never supplies projection inputs. Missing,
stale, malformed, ambiguous, or identity-mismatched data fails closed.

Permanent contract:
- exact official ESPN event/team/athlete IDs;
- no player-name matching or fuzzy identity;
- no synthetic event/player IDs;
- FanDuel canonical two-way Rushing Yards only;
- market context only; projection weight = 0.0;
- probability/fair odds/EV/grading/stake sizing/wager actions OFF.
"""
...
MODEL_VERSION = "NFL RUSHING YARDS KYRE SPORTS API MARKET CLIENT V1"
...
def _fail(reason: str, *, event_id: str = "", athlete_id: str = "", http: int | None = None) -> dict:
    return {
        "ready": False,
        "reason": _safe(reason, "Kyre Sports API Rushing Yards market unavailable"),
...
def validate_event_payload(
    ...
    *,
    ...
    if _safe(payload.get("schema_version")) != SCHEMA_VERSION:
        return _fail("Kyre Sports API Rushing Yards market schema mismatch", event_id=event_id)
    ...
    if (
        ...
        return _fail("Kyre Sports API Step 4 market safety contract failed closed", event_id=event_id)
    ...
    for raw in rows:
        ...
        if (
            ...
            or _safe(raw.get("market_type")).lower() != "rushing_yards"
...
def fetch_event_market(
    ...
    *,
    ...
    if not event_id.isdigit():
        return _fail("official ESPN event ID is required", event_id=event_id)
    url = f"{_safe(base_url, _api_base_url()).rstrip('/')}/api/v1/nfl/rushing-yards/market"
...
def market_for_athlete(event_market: dict, official_athlete_id: str, official_team_id: str = "") -> dict:
    ...
    if len(matches) != 1:
        return _fail(
            "fresh exact-ID Rushing Yards market unavailable for this player" if not matches else "ambiguous exact-ID Rushing Yards markets for this player",
            event_id=event_id,
            athlete_id=athlete_id,
