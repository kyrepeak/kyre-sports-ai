"""CFB Over/Under Slate V8 — Upgrade Step 9 environment orchestration.

Pipeline:
frozen raw model -> frozen Steps 3-8 -> Step 9 game-day environment ->
unchanged frozen final synthesis.

Step 9 preserves Step-8 projected points and projected total exactly. Verified
weather can widen structural uncertainty; roster injury fields remain audit-only.
"""
...
import cfb_over_under_slate_v7 as frozen

MODEL_VERSION = "CFB OVER/UNDER SLATE V8 • UPGRADE STEP 9 ENVIRONMENT"
FROZEN_SLATE = "cfb_over_under_slate_v7"
...
def analyze_game(
    ...
    engine = environment_engine.build_environment_engine(game, away, home)
    raw = environment_engine.apply_to_raw(base_raw, engine)
    ...
    out.update({
        ...
        "final": dict(final),
        "step8_raw": base_raw,
        "environment_engine": dict(engine),
...
def scan_slate(
    ...
    engine_ready = sum(bool((row.get("environment_engine") or {}).get("model_ready")) for row in rows)
    engine_applied = sum(bool((row.get("raw") or {}).get("upgrade_step9_applied")) for row in rows)
    ...
    return rows, {
        ...
        "environment_engine_ready": engine_ready,
        "environment_engine_applied": engine_applied,
