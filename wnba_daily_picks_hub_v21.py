"""WNBA Daily Picks V21 — Game Total Step-9 seven-market read-only integration.

Renders the frozen V18 Daily Picks production/verification surface, then appends
PRA + Points + Rebounds + Assists + Spread + Moneyline + Game Total integration.
Game Total is read only from its completed same-session Step-8 payload. No source
model runs, network requests, simulations, backfills, or source writes occur here.
"""
...
MODEL_VERSION = "WNBA DAILY PICKS V21 • GAME TOTAL STEP-9 SEVEN-MARKET INTEGRATION"
_ET = ZoneInfo("America/New_York")
_MARKETS = ("PRA", "POINTS", "REBOUNDS", "ASSISTS", "SPREAD", "MONEYLINE", "GAME TOTAL")
...
def _render_v18_with_live_feed_tiles(section_header=None, status_info=None, team_logo=None, h=None):
    ...
    def live_status_card(label, state, note):
        ...
        if key == "GAME TOTAL":
            return original_card(label, str(total.get("state") or "⏳ NOT RUN"), str(total.get("detail") or "No completed same-day Game Total payload."))
        return original_card(label, state, note)

    def live_caption(body, *args, **kwargs):
        text = str(body)
        old_phrases = {
            "PRA, Points and Rebounds remain independent read-only feeds. Assists and game markets remain future connectors.",
            "PRA, Points, Rebounds, Assists and Spread are independent read-only feeds. Moneyline and Game Total remain future connectors.",
            "PRA, Points, Rebounds, Assists, Spread and Moneyline are independent read-only feeds. Game Total remains a future connector.",
        }
        if text in old_phrases:
            body = "PRA, Points, Rebounds, Assists, Spread, Moneyline and Game Total are independent read-only feeds."
        return original_caption(body, *args, **kwargs)

    def live_markdown(body, *args, **kwargs):
        if isinstance(body, str) and "KYRE SPORTS AI • WNBA DAILY PICKS • STEP 9" in body:
            body = body.replace("🔌 3 read-only connectors", "🔌 7 read-only connectors")
            body = body.replace("🔌 5 read-only connectors", "🔌 7 read-only connectors")
            body = body.replace("🔌 6 read-only connectors", "🔌 7 read-only connectors")
...
def render_wnba_daily_picks_hub(section_header=None, status_info=None, team_logo=None, h=None):
    ...
    guarded = seven.evaluate_seven_market(selected, day, feeds=feeds, now_et=now)
    ready = seven.ready_rows(guarded)
    diag = seven.diagnostics(bundle, guarded)
    total = total_feed.status(day)

    gt_common = _count(common, "GAME TOTAL")
    gt_safe = _count(audit, "GAME TOTAL", "Safety state", "SAFE")
    gt_ranked = _count(ranked, "GAME TOTAL", "Rank state", "RANKED")
    gt_selected = _count(selected, "GAME TOTAL")
    gt_ready = _count(ready, "GAME TOTAL")
    ...
    st.markdown("---")
    st.markdown("## 🧮 Step 9 — Game Total → Daily Picks Integration")
    st.caption(
    ...
    a1.metric("Game Total connector", "CONNECTED" if total.get("connected") else "CHECK")
    a2.metric("Game Total source picks", int(total.get("production_picks") or 0))
    a3.metric("Game Total common rows", gt_common)
    a4.metric("Game Total SAFE", gt_safe)

    b1, b2, b3, b4 = st.columns(4)
    b1.metric("Game Total RANKED", gt_ranked)
    b2.metric("Game Total SELECTED", gt_selected)
    b3.metric("Game Total FINAL READY", gt_ready)
    b4.metric("Seven-market feeds", f"{connected_n}/7")

    source_picks = int(total.get("production_picks") or 0)
    connector_pass = bool(
        total.get("connected")
        and gt_common == source_picks
        and gt_safe == gt_common
        and gt_ranked == gt_common
    )
    if connector_pass:
        if source_picks:
            st.success("✅ GAME TOTAL CONNECTOR STEP 9 PASSED • completed Game Total Step-8 QUALIFIED output reconciles read-only through schema, safety and ranking with no source-model changes.")
        else:
            st.success("✅ GAME TOTAL CONNECTOR STEP 9 PASSED • Game Total Step 8 is connected with 0 QUALIFIED plays; Daily Picks correctly adds zero forced Game Total rows.")
    else:
        st.warning("⚠️ GAME TOTAL CONNECTOR CHECK • the same-day Game Total Step-8 source is missing or a row failed schema/safety/ranking reconciliation.")
    ...
    if not selected.empty:
        ...
        def candidate(r):
            ...
            if market == "GAME TOTAL":
                try: return f"{r.get('Side')} {float(r.get('Line')):g}"
                except Exception: return f"{r.get('Side')} game total"
            return f"{r.get('Player')} {r.get('Side')} {r.get('Line')}"
        show["Candidate"] = show.apply(candidate, axis=1)
        cols = [c for c in ("Daily rank", "Rank", "Market", "Candidate", "Book", "Posted odds", "Model probability", "Edge", "Ranking score") if c in show.columns]
        st.markdown("### 🏆 Seven-Market Overall Top 5 — pre-guard")
        st.dataframe(show[cols], use_container_width=True, hide_index=True)

    st.markdown("### 🛡️ Seven-Market Final Production Guard")
    ...
    if isinstance(guarded, pd.DataFrame) and not guarded.empty:
        g = guarded.copy()
        cols = [c for c in ("Daily rank", "Rank", "Market", "Player", "Team", "Side", "Line", "Book", "Guard state", "Guard reasons") if c in g.columns]
        st.dataframe(g[cols], use_container_width=True, hide_index=True)

    d1, d2, d3 = st.columns(3)
    d1.metric("Seven-market pipeline", "PASS" if pipeline else "CHECK")
    d2.metric("All feeds connected", f"{connected_n}/7")
    d3.metric("Final READY", f"{len(ready)}/5")

    if all7 and pipeline:
        st.success("✅ SEVEN-MARKET END-TO-END PASS • PRA + Points + Rebounds + Assists + Spread + Moneyline + Game Total reconcile through one guarded Daily Picks Top-5 pipeline.")
    elif pipeline:
        missing = [m.title() for m in _MARKETS if not connected[m]]
        st.info("ℹ️ Game Total Step 9 is live. Full seven-market end-to-end verification is waiting on: " + ", ".join(missing) + ".")
    else:
        st.warning("⚠️ SEVEN-MARKET PIPELINE CHECK • inspect the guard table before changing any source model. No backfill is performed.")

    st.caption(
        f"⚡ Daily Picks V21 • Game Total Step 9 read-only • checked {now.strftime('%Y-%m-%d %I:%M:%S %p ET')} • "
