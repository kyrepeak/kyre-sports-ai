"""CFB Over/Under Slate V7 — Upgrade Step 8 turnover-volatility orchestration.

Pipeline:
frozen Step 8 raw model -> frozen Step 3 offense-vs-defense ->
frozen Step 4 pace -> frozen Step 5 explosive proxy ->
frozen Step 6 red-zone -> frozen Step 7 third-down drive sustain ->
Step 8 turnover-possession volatility -> unchanged Step 9 synthesis.

Step 8 preserves the Step-7 projected total exactly. Verified turnover evidence
...
import cfb_over_under_slate_v6 as frozen
import cfb_over_under_turnover_engine_v1 as turnover_engine

MODEL_VERSION = "CFB OVER/UNDER SLATE V7 • UPGRADE STEP 8 TURNOVER VOLATILITY"
FROZEN_SLATE = "cfb_over_under_slate_v6"
...
def analyze_game(
    ...
    raw = turnover_engine.apply_to_raw(base_raw, engine)
...
def scan_slate(
    ...
    engine_ready = sum(
        bool((row.get("turnover_engine") or {}).get("model_ready"))
        for row in rows
    )
    engine_applied = sum(
        bool((row.get("raw") or {}).get("upgrade_step8_applied"))
        for row in rows
    )
    ...
    return rows, {
        ...
        "turnover_engine_ready": engine_ready,
        "turnover_engine_applied": engine_applied,
