'''Kyre Sports AI entrypoint — MLB V2.1.7 frozen + WNBA PRA V3.2.1 frozen + Points V1.5.1 active.
...
WNBA Points V1.5.1 uses the isolated ET-reconciled WNBA schedule V2.5 plus its own
corrected 8-team player pool and 4-game matchup context. PRA totals never feed the
Points projection and Points is not yet fed into the shared WNBA Final Card.

The deep-shell loader patch is deliberately narrow: it changes only inherited
WNBA Points routing. Legacy Points import names are pinned to the cache-safe V1.5.1
module so Streamlit cannot revive stale page modules through older wrappers. MLB
model math, PRA model math and all frozen connectors remain unchanged.
'''
...
import wnba_points_hub_v151 as wnba_points_v151
...
_NEW_WNBA_PLACEHOLDER = '''    elif market == "Points":
        from wnba_points_hub_v151 import render_wnba_points_hub
...
def _patch_inherited_app_text(value):
    ...
    for old_module in (
        ...
        "wnba_points_hub_v14", "wnba_points_hub_v15",
    ):
        text = text.replace(
    ...
    if "wnba_points_hub_v151" not in text and _OLD_WNBA_PLACEHOLDER in text:
...
sys.modules["wnba_points_hub_v11"] = wnba_points_v151
sys.modules["wnba_points_hub_v12"] = wnba_points_v151
sys.modules["wnba_points_hub_v13"] = wnba_points_v151
sys.modules["wnba_points_hub_v14"] = wnba_points_v151
sys.modules["wnba_points_hub_v15"] = wnba_points_v151
...
exec(
    compile(source, "kyre_sports_ai_mlb_v217_wnba_pra_v321_frozen_points_v151_full_handoff.py", "exec"),
    globals(),
    globals(),
)
