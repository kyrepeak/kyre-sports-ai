'''Kyre Sports AI entrypoint — Daily Picks V22 + Assists V20 + Points preflight repair + PRA V3.6.1 speed route + WNBA Spread V1.6.1 + Moneyline V1.5 + Game Total V1.5.
...
1) WNBA Daily Picks' historical V4 import is rebound to Daily Picks V22. V22
   preserves the complete V21/V18/V17 production/verification surface and all
   seven existing read-only market integrations, then adds only the Step-1
   seven-market master-controller shell above Daily Picks. The Run All 7 button
   is intentionally disabled in Step 1 and launches zero source models,
   simulations, network requests, backfills, writes, or ranking changes.
...
6) WNBA Moneyline is added at the runtime WNBA selectbox boundary and its unfinished
   ...
   existing ±5% sensitivity. No play is forced. Daily Picks V22 consumes Step 8
...
7) The unfinished WNBA Game Total fallback opens isolated Game Total V1.5. V1.5
   ...
   sensitivity. No play is forced. Daily Picks V22 consumes only the completed
...
import wnba_daily_picks_hub_v22 as wnba_daily_picks_v22
...
sys.modules["wnba_daily_picks_hub_v4"] = wnba_daily_picks_v22
...
exec(
    compile(
        source,
        "kyre_sports_ai_preserved_app_plus_daily_picks_v22_controller_shell_assists_v20_points_v19845_pra_v361_spread_v161_moneyline_v15_game_total_v15_runtime_nav.py",
        "exec",
    ),
    globals(),
    globals(),
