'''Kyre Sports AI entrypoint — exact frozen MLB/WNBA production + isolated NFL V1.4 runtime route.
...
3. an isolated NFL Sport/Market navigation row and nfl_hub_v14 page are rendered;
4. selecting MLB or WNBA again returns directly to the untouched frozen app.

NFL V1.4 preserves the verified Slate V1 foundation and Moneyline Steps 1-3.6,
while adding Step 4A historical team-strength baselines. Step 3 remains a final-
output preseason safety gate instead of blocking model development. Step 4A uses
completed regular-season results only, with prior-season carryover and gradual
current-season shrinkage. Its 0-100 strength index is descriptive/internal and is
NOT a calibrated win probability. Sportsbook pricing, final P(win), Monte Carlo,
ranking, edge/EV and recommendations remain OFF. All other NFL markets remain
reserved. MLB/WNBA production remains frozen and untouched.
'''
...
if str(st.session_state.get("ks_sport_touch") or "").upper() == "NFL":
    ...
    from nfl_hub_v14 import render_nfl_hub
...
exec(
    compile(
        source,
        "kyre_sports_ai_frozen_pre_nfl_plus_direct_runtime_nfl_v14_moneyline_step4a_team_strength.py",
        "exec",
    ),
    globals(),
    globals(),
