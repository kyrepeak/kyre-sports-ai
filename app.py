...
# Runtime cache-bust for the persistent Final Decision dashboard. A long-lived
# Streamlit process may still hold an older V3.6 module object that referenced
# the Step-2 connector directly. Rebind only that connector reference and
# reinstall the Final Decision read/UI hooks. This does not run a model, regrade
# a row, or touch Rebounds/MLB.
wnba_pra_v36.points_final_connector = wnba_final_points_v3
try:
    wnba_pra_v321._impl.points_final_connector = wnba_final_points_v3
except Exception:
    pass
wnba_final_points_v3.install()

# Frozen MLB sportsbook routing stays exactly as before.
slate_multi_provider.install()

exec(
    compile(source, "kyre_sports_ai_mlb_v217_wnba_pra_v321_points_v19841_rebounds_v12_step3_finalpoints_v3.py", "exec"),
    globals(),
    globals(),
)
