'''Kyre Sports AI entrypoint — exact frozen MLB/WNBA production + isolated NFL V1.4.1 runtime route.
...
3. an isolated NFL Sport/Market navigation row and nfl_hub_v141 page are rendered;
4. selecting MLB or WNBA again returns directly to the untouched frozen app.

NFL V1.4.1 preserves the verified Slate V1 foundation and Moneyline Steps 1-3.6,
plus Step 4A's exact team-strength formula. V4.1 repairs only historical regular-
season retrieval: ESPN team schedule remains primary, while an ESPN league-wide
week-by-week regular-season reconstruction is used when the primary path returns
insufficient usable history. Preseason results remain excluded. Step 3 remains a
final-output preseason safety gate. Sportsbook pricing, calibrated P(win), Monte
Carlo, ranking, edge/EV and recommendations remain OFF. All other NFL markets
remain reserved. MLB/WNBA production remains frozen and untouched.
'''
...
if str(st.session_state.get("ks_sport_touch") or "").upper() == "NFL":
    ...
    from nfl_hub_v141 import render_nfl_hub
...
exec(
    compile(
        source,
        "kyre_sports_ai_frozen_pre_nfl_plus_direct_runtime_nfl_v141_moneyline_step4a_historical_fallback.py",
        "exec",
    ),
    globals(),
    globals(),
