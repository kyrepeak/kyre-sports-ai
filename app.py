"""Kyre Sports AI Streamlit entrypoint — NFL Passing Yards route hotfix V2.

Router V96 is additive over certified Router V95. It fixes the runtime precedence
bug that let V91 overwrite the newer V92–V95 Passing Yards handlers, which is why
production still showed the old Step 8/9/10 ACTIVE banners after cleanup Steps 1–4.

V96 changes only the routing overwrite point. NFL Passing Yards now reaches V16's
matchup spotlight / why-this-projection presentation. No loader, projection,
context, distribution, probability, market, grading, CFB, or other sport logic
changes. Sportsbook projection influence remains 0.0% and stake sizing remains OFF.

Deployment heartbeat:
STREAMLIT_MAIN_V96_NFL_PASSING_YARDS_ROUTE_PRECEDENCE_HOTFIX_V2_2026-09-11.
"""
...
DEPLOYMENT_HEARTBEAT = "STREAMLIT_MAIN_V96_NFL_PASSING_YARDS_ROUTE_PRECEDENCE_HOTFIX_V2_2026-09-11"
