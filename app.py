'''Kyre Sports AI entrypoint — MLB V2.1.7 frozen + WNBA PRA V3.2.1 frozen + Points V1.7.1 active.
...
WNBA Points V1.7.1 uses the ET-reconciled WNBA schedule V2.5, strict current-roster
gate, corrected 8-team player/context handoff, explicit Points-date empirical
game logs and a synchronized projection-sanity preflight. Established matched
players must have verified >=5-game scoring history, and unresolved >35%
projection-vs-own-history deviations block 5M until reviewed. Sportsbook lines
never drive that sanity gate. PRA totals never feed the Points projection and
Points is not yet fed into the shared WNBA Final Card.
...
V1.7.1 module. MLB model math, PRA model math and frozen connectors remain
unchanged.
'''
...
import wnba_points_hub_v171 as wnba_points_v171
...
_NEW_WNBA_PLACEHOLDER = '''    elif market == "Points":
        from wnba_points_hub_v171 import render_wnba_points_hub
...
def _patch_inherited_app_text(value):
    ...
    for old_module in (
        ...
        "wnba_points_hub_v16", "wnba_points_hub_v17",
    ):
        text = text.replace(
    ...
    if "wnba_points_hub_v171" not in text and _OLD_WNBA_PLACEHOLDER in text:
...
sys.modules["wnba_points_hub_v11"] = wnba_points_v171
sys.modules["wnba_points_hub_v12"] = wnba_points_v171
sys.modules["wnba_points_hub_v13"] = wnba_points_v171
sys.modules["wnba_points_hub_v14"] = wnba_points_v171
sys.modules["wnba_points_hub_v15"] = wnba_points_v171
sys.modules["wnba_points_hub_v151"] = wnba_points_v171
sys.modules["wnba_points_hub_v16"] = wnba_points_v171
sys.modules["wnba_points_hub_v17"] = wnba_points_v171
...
exec(
    compile(source, "kyre_sports_ai_mlb_v217_wnba_pra_v321_frozen_points_v171_sanity_gate.py", "exec"),
    globals(),
    globals(),
)
