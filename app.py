'''Kyre Sports AI entrypoint — MLB V2.1.7 frozen + WNBA PRA V3.2.1 frozen + Points V1.5 active.
...
WNBA Points V1.5 uses the isolated ET-reconciled WNBA schedule V2.5 plus its own
...
WNBA Points routing. Legacy Points import names are pinned to V1.5 so Streamlit
...
import wnba_points_hub_v15 as wnba_points_v15
...
_NEW_WNBA_PLACEHOLDER = '''    elif market == "Points":
        from wnba_points_hub_v15 import render_wnba_points_hub
...
def _patch_inherited_app_text(value):
    ...
    if "wnba_points_hub_v15" not in text and _OLD_WNBA_PLACEHOLDER in text:
...
sys.modules["wnba_points_hub_v11"] = wnba_points_v15
sys.modules["wnba_points_hub_v12"] = wnba_points_v15
sys.modules["wnba_points_hub_v13"] = wnba_points_v15
sys.modules["wnba_points_hub_v14"] = wnba_points_v15
...
exec(
    compile(source, "kyre_sports_ai_mlb_v217_wnba_pra_v321_frozen_points_v15_full_handoff.py", "exec"),
    globals(),
    globals(),
)
