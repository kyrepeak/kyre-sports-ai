'''Kyre Sports AI entrypoint — exact frozen MLB/WNBA production + isolated NFL V1.3 runtime route.
...
3. an isolated NFL Sport/Market navigation row and nfl_hub_v13 page are rendered;
4. selecting MLB or WNBA again returns directly to the untouched frozen app.

NFL V1.3 preserves the verified Slate V1 foundation and advances only Moneyline
to Step 3: verified pregame slate/clock + current ESPN QB depth + current ESPN
injuries + fresh explicit preseason game-plan evidence with conflict/freshness
guards. NFL sportsbook pricing, projection math, Monte Carlo, ranking and
...
if str(st.session_state.get("ks_sport_touch") or "").upper() == "NFL":
    ...
    from nfl_hub_v13 import render_nfl_hub
...
exec(
    compile(
        source,
        "kyre_sports_ai_frozen_pre_nfl_plus_direct_runtime_nfl_v13_moneyline_step3.py",
        "exec",
    ),
    globals(),
    globals(),
