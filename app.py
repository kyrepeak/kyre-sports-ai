'''Kyre Sports AI entrypoint — Daily Picks V19 + Assists V20 + Points preflight repair + PRA V3.6.1 speed route + WNBA Spread V1.6.1 + Moneyline V1.2.
...
6) WNBA Moneyline is added at the runtime WNBA selectbox boundary and its unfinished
   fallback opens isolated Moneyline V1.2. V1.2 preserves the verified V1.0/V1.1
   Eastern-date slate, clock-safe pregame eligibility, team context, exact-day
   availability and exact same-book two-sided sportsbook Moneyline verification,
   then adds Step 5 market-independent win probability using verified team scoring,
   venue, pace/efficiency, availability and date-cut empirical margin uncertainty.
   Sportsbook prices gate Step 5 but are not model inputs. No-vig/fair odds,
...
import wnba_moneyline_hub_v12 as wnba_moneyline_v12
...
def _wnba_market_route_info(body, *args, **kwargs):
    ...
    if text.startswith("WNBA Moneyline is separate from") and unfinished:
        wnba_moneyline_v12.render_wnba_moneyline_hub(None, None, None, None)
        st.stop()
...
exec(
    compile(
        source,
        "kyre_sports_ai_preserved_app_plus_daily_picks_v19_assists_v20_points_v19845_pra_v361_spread_v161_moneyline_v12_runtime_nav.py",
        "exec",
    ),
    globals(),
    globals(),
