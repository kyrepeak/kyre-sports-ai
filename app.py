"""Kyre Sports AI — WNBA Live Games Step-6.4 PBP-rich calibration wrapper.
...
isolated WNBA Live Games implementation target:
    ...
    Live Games -> wnba_live_hub_v64

The verified production Steps 1-6, Step-6.1 replay harness, Step-6.2 rejected
score/clock-only calibration, and Step-6.3 PBP fidelity audit remain intact.
V6.4 appends a PBP-rich walk-forward calibration lab. No candidate is
...
replacement = "    import wnba_live_hub_v64 as wnba_live_v1"
if anchor not in source:
    raise RuntimeError("Frozen WNBA Live Step-1 route import not found.")
source = source.replace(anchor, replacement, 1)

compile(source, "<kyre_wnba_live_step64_pbp_calibration_preflight>", "exec")
exec(
    compile(source, "kyre_wnba_live_games_v64_pbp_calibration.py", "exec"),
    globals(),
    globals(),
)
