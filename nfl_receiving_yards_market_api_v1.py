"""Fail-closed Kyre Sports API client for NFL Receiving Yards Step 8 market context.

This client is deliberately post-projection. It accepts only a fresh exact-ID
FanDuel Receiving Yards payload and never supplies projection inputs. Missing,
...
MODEL_VERSION = "NFL RECEIVING YARDS KYRE SPORTS API MARKET CLIENT V1"
...
def _line_ok(value: Any) -> bool:
    number = _num(value)
    return _finite(number) and 0 < number <= 500


def _fail(reason: str, *, event_id: str = "", athlete_id: str = "", http: int | None = None) -> dict[str, Any]:
    return {
        "ready": False,
        "reason": _safe(reason, "Kyre Sports API Receiving Yards market unavailable"),
...
def validate_event_payload(payload: Any, official_event_id: str, *, now_utc: datetime | None = None) -> dict[str, Any]:
    ...
    if _safe(payload.get("schema_version")) != SCHEMA_VERSION:
        return _fail("Kyre Sports API Receiving Yards market schema mismatch", event_id=event_id)
    ...
    if not isinstance(identity, dict) or not isinstance(semantics, dict):
        return _fail("Kyre Sports API market safety contract missing", event_id=event_id)
    if (
        ...
        return _fail("Kyre Sports API Step 8 market safety contract failed closed", event_id=event_id)
    ...
    for raw in rows:
        ...
        if (
            ...
            or _safe(raw.get("market_type")).lower() != "receiving_yards"
...
def fetch_event_market(official_event_id: str, *, now_utc: datetime | None = None, base_url: str | None = None) -> dict[str, Any]:
    event_id = _safe(official_event_id)
    if not event_id.isdigit():
        return _fail("official ESPN event ID is required", event_id=event_id)
    url = f"{_safe(base_url, _api_base_url()).rstrip('/')}/api/v1/nfl/receiving-yards/market"
...
def market_for_athlete(event_market: dict[str, Any], official_athlete_id: str, official_team_id: str = "") -> dict[str, Any]:
    athlete_id = _safe(official_athlete_id)
    team_id = _safe(official_team_id)
    ...
    if team_id and not team_id.isdigit():
        return _fail("official ESPN team ID is invalid", event_id=event_id, athlete_id=athlete_id)
    if not (event_market or {}).get("ready"):
        return _fail(
            _safe((event_market or {}).get("reason"), "Kyre Sports API event market unavailable"),
            event_id=event_id,
            athlete_id=athlete_id,
            http=(event_market or {}).get("http"),
        )
    matches = [
    ...
    if len(matches) != 1:
        return _fail(
            "fresh exact-ID Receiving Yards market unavailable for this player" if not matches else "ambiguous exact-ID Receiving Yards markets for this player",
            event_id=event_id,
            athlete_id=athlete_id,
