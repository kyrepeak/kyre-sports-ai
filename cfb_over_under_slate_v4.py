"""CFB Over/Under Slate V4 — Upgrade Step 5 explosive-play orchestration.

Pipeline:
frozen Step 8 -> frozen Step 3 offense-vs-defense -> frozen Step 4 pace ->
Step 5 explosive-efficiency proxy -> unchanged Step 9 synthesis.

The analysis line remains a comparison threshold only and has 0% influence on
Step-5 explosive evidence or projection adjustment.
"""
...
import cfb_over_under_slate_v3 as frozen

MODEL_VERSION = "CFB OVER/UNDER SLATE V4 • UPGRADE STEP 5 EXPLOSIVE"
FROZEN_SLATE = "cfb_over_under_slate_v3"
...
def analyze_game(
    ...
    raw = explosive_engine.apply_to_raw(base_raw, engine)
...
def scan_slate(
    ...
    engine_ready = sum(
        bool((row.get("explosive_engine") or {}).get("model_ready"))
        for row in rows
    )
    engine_applied = sum(
        bool((row.get("raw") or {}).get("upgrade_step5_applied"))
        for row in rows
    )
    ...
    return rows, {
        ...
        "explosive_engine_ready": engine_ready,
        "explosive_engine_applied": engine_applied,
