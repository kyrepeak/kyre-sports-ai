"""KYRE Streamlit Router V158 — CFB Game Total V162 page-cache boundary.

Additive over frozen Router V157. V158 fixes one production-only presentation
regression: the root lazy-router purge is route-token based, so staying on the
same College Football -> Game Total route can retain stale
cfb_game_total_clean_page_* module objects across a long-lived Streamlit
process. V158 unconditionally evicts only that Game Total page-module family
before importing fresh V13. Every non-target route delegates through V157.
...
import streamlit_memory_lazy_router_v157 as prior
...
def _purge_game_total_page_modules() -> int:
    """Always evict stale Game Total presentation modules before exact-route import."""
    removed = 0
    for name in list(sys.modules):
        if name.startswith(GAME_TOTAL_PAGE_PREFIX):
...
def _render_cfb_game_total_v158(market: str) -> None:
    sport = str(st.session_state.get("ks_sport_touch") or "")
