"""KYRE Streamlit Router V132 — direct NFL Spread API route.

V132 owns only exact NFL -> Spread. Every non-Spread route delegates to frozen
Router V131 unchanged, preserving the certified Moneyline V12 fast route and the
entire older router chain beneath it.
"""
...
MODEL_VERSION = "KYRE STREAMLIT ROUTER V132 • NFL SPREAD API DIRECT ROUTE"
FROZEN_ROUTER = "streamlit_memory_lazy_router_v131"
NFL_SPORT_LABEL = "NFL"
SPREAD_MARKET = "Spread"
ACTIVE_SPREAD_HUB = "nfl_spread_hub_v1"
...
def _spread_query_active() -> bool:
    return (
        _query_value(ROUTE_QUERY_SPORT) == NFL_SPORT_LABEL
        and _query_value(ROUTE_QUERY_MARKET) == SPREAD_MARKET
    )


def _clear_spread_route_query() -> None:
    if not _spread_query_active():
        return
    try:
...
def _persist_spread_route_query() -> None:
    try:
        if _query_value(ROUTE_QUERY_SPORT) != NFL_SPORT_LABEL:
            st.query_params[ROUTE_QUERY_SPORT] = NFL_SPORT_LABEL
        if _query_value(ROUTE_QUERY_MARKET) != SPREAD_MARKET:
            st.query_params[ROUTE_QUERY_MARKET] = SPREAD_MARKET
    except Exception:
        pass


def _restore_spread_route_from_query() -> bool:
    ...
    st.session_state["ks_nfl_market_touch"] = SPREAD_MARKET
    return True
...
def _fast_route_active() -> bool:
    return (
        ...
        and str(st.session_state.get("ks_nfl_market_touch") or "") == SPREAD_MARKET
    )


def _selectbox_v132(label: Any, options: Any, *args: Any, **kwargs: Any):
    selected = _ORIGINAL_SELECTBOX(label, options, *args, **kwargs)
    if label == "🏟️ Sport" and str(selected) != NFL_SPORT_LABEL:
        _clear_spread_route_query()
    elif label == "🎯 NFL Market":
        if str(selected) == SPREAD_MARKET:
            _persist_spread_route_query()
        else:
            _clear_spread_route_query()
    return selected


def _render_nfl_v132(market: str) -> None:
    market = str(market or "Slate")
    if market != SPREAD_MARKET:
        raise RuntimeError("Router V132 direct handler is Spread only.")

    _persist_spread_route_query()
    legacy_chain_skipped = FROZEN_ROUTER not in sys.modules
    page_import_started = perf_counter()
    module = root._import(ACTIVE_SPREAD_HUB)
    page_import_ms = (perf_counter() - page_import_started) * 1000.0

    try:
        st.session_state["nfl_spread_cold_start_v132_last"] = {
            ...
            "restored_from_query": _spread_query_active(),
            "frozen_router": FROZEN_ROUTER,
            "active_page": ACTIVE_SPREAD_HUB,
            "sportsbook_projection_influence": 0.0,
            "stake_sizing_enabled": False,
            "wager_actions_enabled": False,
        }
...
def _render_direct_spread() -> None:
    ...
    root.st.selectbox = _selectbox_v132
    root._render_nfl = _render_nfl_v132
