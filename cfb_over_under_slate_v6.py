"""CFB Over/Under Slate V6 — Upgrade Step 7 third-down drive-sustain orchestration.

Pipeline:
frozen Step 8 -> frozen Step 3 offense-vs-defense -> frozen Step 4 pace ->
frozen Step 5 explosive proxy -> frozen Step 6 red-zone ->
Step 7 third-down drive-sustain -> unchanged Step 9 synthesis.

The analysis line remains a comparison threshold only and has 0% influence on
Step-7 third-down evidence or projection adjustment.
"""
...
import cfb_over_under_slate_v5 as frozen
import cfb_over_under_third_down_engine_v1 as third_down_engine

MODEL_VERSION = "CFB OVER/UNDER SLATE V6 • UPGRADE STEP 7 THIRD DOWN"
FROZEN_SLATE = "cfb_over_under_slate_v5"
...
def analyze_game(
    ...
    raw = third_down_engine.apply_to_raw(base_raw, engine)
...
def scan_slate(
    ...
    engine_ready = sum(
        bool((row.get("third_down_engine") or {}).get("model_ready"))
        for row in rows
    )
    engine_applied = sum(
        bool((row.get("raw") or {}).get("upgrade_step7_applied"))
        for row in rows
    )
    ...
    return rows, {
        ...
        "third_down_engine_ready": engine_ready,
        "third_down_engine_applied": engine_applied,
