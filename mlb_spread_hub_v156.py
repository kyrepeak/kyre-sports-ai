"""MLB Spread / Run Line V15.6.2 — Step-1 H2H date/source repair.

This wrapper preserves the exact V15.6 Step-1 card from commit
8f44cf7da6e678be1371452f7db2de985c27c656, keeps the V15.6.1 direct MLB-logo
transport, and fixes the historical-game intake used by the existing V15.2 H2H
adjustment.

Root cause
----------
The legacy ``spread_history.h2h_last10`` asks MLB StatsAPI for one multi-season
schedule range (for example 2022-01-01 through yesterday). On the active scanner
that query can return an incomplete/stale slice of the team's schedule. The card
therefore showed 2022 meetings even when newer completed H2H games existed.

Repair
------
Fetch H2H history one MLB season at a time, newest season first, normalize the
team pair so each matchup is requested only once, stop after enough recent H2H
games are collected, then convert those games to the selected team's perspective.
...
remain unchanged. Because the *source games* are corrected, a fresh scan can
...
MODEL_VERSION = "V15.6.2 • TOP-5 CARD STEP 1 • FRESH H2H SOURCE"
_HISTORY_SOURCE_TOKEN = "mlb_spread_h2h_segmented_v1562"
...
def _fresh_pair_games(team_a: int, team_b: int, years_back: int = 4) -> list[dict]:
    ...
    can yield an incomplete/stale schedule slice. We stop once at least 10 recent
    ...
    games = sorted(found.values(), key=lambda x: x["date"], reverse=True)
    return games[:10]
...
def _invalidate_legacy_saved_scan_once():
    """Prevent stale 2022-era H2H payloads from surviving the source repair."""
...
def render_spread_hub(games_df, section_header, status_info, team_logo, h):
    """Render frozen V15.6 with direct logos + corrected newest-first H2H intake."""
    ...
    try:
        st.caption(
            "🛠️ MLB Spread V15.6.2 • H2H source repaired: newest completed meetings "
            "are fetched season-by-season from MLB StatsAPI • model formula unchanged."
        )
        if refreshed:
            st.info(
                "H2H history source was repaired, so the saved pre-repair spread scan "
                "was cleared. Run the V15.2 spread scan once to rebuild probabilities "
                "from the corrected recent H2H games."
            )
