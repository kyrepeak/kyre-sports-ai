"""CFB Over/Under Slate V9 — Upgrade Step 10 historical context orchestration.

Pipeline:
frozen Step 9 -> Step 10 pre-kickoff history audit -> unchanged frozen final synthesis.

Step 10 does not move projected points, projected total, structural uncertainty,
reliability, qualification thresholds, or analysis-line behavior.
"""
...
import cfb_over_under_slate_v8 as frozen

MODEL_VERSION = "CFB OVER/UNDER SLATE V9 • UPGRADE STEP 10 HISTORY"
FROZEN_SLATE = "cfb_over_under_slate_v8"
...
def analyze_game(
    ...
    step9_environment = dict(base.get("environment_engine") or {})

    engine = history_engine.build_history_engine(
        game, away, home, step9_environment=step9_environment
    )
    raw = history_engine.apply_to_raw(base_raw, engine)
...
def scan_slate(
    ...
    engine_ready = sum(bool((row.get("history_engine") or {}).get("model_ready")) for row in rows)
    engine_applied = sum(bool((row.get("raw") or {}).get("upgrade_step10_applied")) for row in rows)
    ...
    return rows, {
        ...
        "history_engine_ready": engine_ready,
        "history_engine_applied": engine_applied,
