"""MLB Matchup Explorer V1.1 — player-first research + embedded frozen engines.

Reads, never rewrites, the production models:
- 1+ Hit V13.3 / V13 deep model
- Home Run V1.1 calibrated model
- H+R+RBI V1.0.1 joint-event model

Total Bases, RBI and Runs remain descriptive recent-performance tabs until they
receive their own isolated production models. Pregame projections are clearly
labeled when the selected game has already started.
"""
...
def _selected_candidate(games_df, row, player_id):
    try:
        pool, _ = hit133._candidate_pool(games_df, include_live=True)
    except Exception:
        pool = []
    ...
    for c in pool:
        try:
            if int(c.get("game_pk")) == pk and int(c.get("player_id")) == pid:
                return c
        except Exception:
            continue
    return None
...
def _hit_projection(c):
    try:
        r = hit133._downgrade_projected(hitcore.deep_scan(dict(c), 250_000))
        return r
...
def _hr_projection(c):
    try:
        r = hrcore._model_candidate(dict(c), deep=True, sims=250_000)
        return r or {"error":"No calibrated HR profile returned."}
    except Exception as exc:
        return {"error": f"{type(exc).__name__}: {exc}"}
...
def _hrr_projection(c):
    try:
        ...
        if not r:
            return {"error":"No H+R+RBI profile returned."}
        r = dict(r)
        r["sim"] = hrrcore._simulate(r, 250_000)
        return r
    except Exception as exc:
        return {"error": f"{type(exc).__name__}: {exc}"}
...
def _render_hit_card(r, started=False):
    if r.get("error"):
        st.warning(f"1+ Hit engine unavailable for this player: {r['error']}")
        return
    s = r.get("sim") or {}
    p = _finite(s.get("p_one_plus"))
    if started:
        st.markdown('<div class="mx-warn">⚠️ Pregame-model view: this game has already started. This is not a live hit-prop projection.</div>', unsafe_allow_html=True)
...
def _render_hr_card(r, started=False):
    if r.get("error"):
        st.warning(f"Home Run engine unavailable for this player: {r['error']}")
        return
    p = _finite(r.get("p_hr"))
    sc = r.get("statcast") or {}
    barrel = _finite(sc.get("barrel_rate"), -1)
    hr9 = r.get("pitcher_hr9")
    if started:
        st.markdown('<div class="mx-warn">⚠️ Pregame-model view: this game has already started. This is not a live home-run projection.</div>', unsafe_allow_html=True)
    st.markdown(f'''<div class="mx-proj"><div class="mx-top"><div class="mx-engine">💣 Home Run V1.1 calibrated model</div><div class="mx-badge">250K simulations</div></div><div class="mx-big">{p*100:.1f}%</div><div class="mx-label">1+ HR probability • Fair {odds(p)}</div><div class="mx-grid"><div class="mx-cell"><span>2+ HR</span><b>{_finite(r.get('p_2hr'))*100:.1f}%</b></div><div class="mx-cell"><span>Season HR</span><b>{r.get('season_hr','—')}</b></div><div class="mx-cell"><span>Barrel%</span><b>{barrel*100:.1f}%</b></div><div class="mx-cell"><span>Starter HR/9</span><b>{float(hr9):.2f}</b></div></div><div class="mx-conf">{r.get('confidence','—')}</div></div>'''.replace(f'{barrel*100:.1f}%', '—' if barrel < 0 else f'{barrel*100:.1f}%').replace(f'{float(hr9):.2f}' if hr9 is not None else 'nan', '—' if hr9 is None else f'{float(hr9):.2f}'), unsafe_allow_html=True)


def _render_hrr_card(r, started=False):
    if r.get("error"):
        st.warning(f"H+R+RBI engine unavailable for this player: {r['error']}")
        return
    s = r.get("sim") or {}
    p2 = _finite(s.get("p2"))
    if started:
        st.markdown('<div class="mx-warn">⚠️ Pregame-model view: this game has already started. This is not a live H+R+RBI projection.</div>', unsafe_allow_html=True)
...
def render_matchup_hub(games_df, section_header=None, status_info=None, team_logo=None, h=None):
    ...
    day = ui._date_str(games_df.iloc[0])
    st.markdown('<div class="mh-hero"><div class="mh-eyebrow">KYRE SPORTS AI • DAILY PLAYER MATCHUPS • V1.1</div><div class="mh-title">⚾ MLB Matchup Explorer</div><div class="mh-sub">One matchup page for recent performance plus the actual frozen 1+ Hit, Home Run and H+R+RBI projection engines.</div></div>', unsafe_allow_html=True)
    labels = [ui._game_label(r) for _, r in games_df.iterrows()]
    idx = st.selectbox("TODAY'S MATCHUPS", range(len(labels)), format_func=lambda i: labels[i], key="mh11_game")
    ...
    if not players:
        st.warning("No eligible hitters were returned for this matchup yet."); return
    pidx = st.selectbox("PLAYER", range(len(players)), format_func=lambda i: f"{players[i]['name']} • {players[i]['team']} • {players[i].get('source','')}", key="mh11_player")
    p = players[int(pidx)]
    ...
    candidate = _selected_candidate(games_df, row, p["id"])
    started = _status_started(row.get("status"))
    ...
    for tab, metric, label, field in ((tabs[2],"TB","Total Bases","totalBases"),(tabs[3],"RBI","RBIs","rbi"),(tabs[4],"R","Runs","runs")):
        with tab:
            st.caption(f"{season} season {label}: {stat.get(field,'—')}"); ui._recent_chart(logs, metric)
            st.info(f"{label} is research-only in Matchup Explorer V1.1. No standalone {label} projection engine has been approved yet, so V1.1 does not manufacture one.")
    with tabs[5]:
    ...
    st.caption(f"{VERSION} • Official MLB data • frozen production engines embedded read-only • selected slate {day}")
