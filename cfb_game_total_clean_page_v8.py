...
Presentation-only wrapper over frozen V7. The main decision path is now one
continuous read: Steps 1–10 -> Step 11 Distribution -> Step 12 Final -> Top-5.
Frozen Game Total analysis, qualification, distribution, final math, ranking,
and sportsbook influence remain untouched. Deep/raw evidence is kept below the
main flow in collapsed drawers.
"""
...
.gt157-model{margin:8px 0 2px;padding-top:7px;border-top:1px solid rgba(121,146,169,.12)}.gt157-model-head{display:flex;align-items:flex-end;justify-content:space-between;gap:8px;margin:0 2px 6px}.gt157-model-head b{color:#edf4fa;font-size:.50rem;font-weight:950;letter-spacing:.06em}.gt157-model-head span{color:var(--gt-gray);font-size:.27rem}
.gt157-stage{display:grid;grid-template-columns:30px minmax(0,1fr) auto;gap:8px;align-items:center;min-height:52px;padding:8px 9px;margin-top:5px;border-radius:11px;background:#0a1823;border:1px solid rgba(116,145,170,.15);position:relative;overflow:hidden}.gt157-stage:before{content:"";position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--gt-purple)}.gt157-stage.ready{border-color:rgba(60,207,137,.24);background:linear-gradient(145deg,rgba(17,72,51,.15),#0a1823)}.gt157-stage.gated{border-color:rgba(244,191,77,.23);background:linear-gradient(145deg,rgba(101,72,18,.12),#0a1823)}
...
.gt157-top5{margin:9px 0 2px;padding:9px;border-radius:12px;background:#0a1722;border:1px solid rgba(116,145,170,.16)}.gt157-top5head{display:flex;align-items:center;justify-content:space-between;gap:8px}.gt157-top5head b{color:#eef5fb;font-size:.50rem;font-weight:950;letter-spacing:.06em}.gt157-top5head span{color:var(--gt-gray);font-size:.27rem}.gt157-deep{margin-top:8px;padding-top:7px;border-top:1px solid rgba(121,146,169,.12);color:var(--gt-gray);font-size:.28rem}
...
def _model_flow(raw: Mapping[str, Any], final: Mapping[str, Any]) -> str:
    """Re-present frozen Step 11/12 fields as two sequential flow stages."""
...
<div class="gt157-model" data-testid="gt157-model-flow">
  ...
  <div class="gt157-stage {'ready' if step11_ready else 'gated'}" data-testid="gt157-step11-card">
    <div class="gt157-num">11</div><div class="gt157-copy"><b>Distribution</b><span>{escape(step11_reason)}</span></div>
  ...
  <div class="gt157-stage {'ready' if step12_ready else 'gated'}" data-testid="gt157-step12-card">
    <div class="gt157-num">12</div><div class="gt157-copy"><b>Final Synthesis</b><span>{escape(step12_reason)}</span></div>
...
def _compact_model_summary(raw: Mapping[str, Any], final: Mapping[str, Any]) -> str:
    """Backward-compatible presenter alias for V8 presentation contracts."""
    return _V157_CSS + _model_flow(raw, final)


def _render_top5(games, selected_day: str) -> None:
    st.markdown(
        '<div class="gt157-top5" data-testid="gt157-top5-stage"><div class="gt157-top5head"><b>🏆 TOP-5 • FINAL FLOW</b><span>Frozen Step-12 qualification + ranking</span></div></div>',
...
def render_game_total_hub(section_header=None, status_info=None, team_logo=None, h=None) -> None:
    """Render one connected UI while reusing the exact frozen V6 model path."""
    st.markdown(
        v5.prior.prior.prior.prior.prior._CSS
        + v5.prior.prior.prior.prior._V151_CSS
        + v5._V152_EVIDENCE_CSS
        + v6._V152_MONSTER_CSS
    ...
    st.markdown(
        """
<div class="gt152-shell"><div class="gt152-shelltop">
...
""",
    ...
    selected = st.date_input(
        ...
        value=datetime.now(v5.prior.prior.prior.prior.prior._PHOENIX).date(),
    ...
    # FROZEN MODEL PATH: same call, same untouched selected game.
    ...
    identity = v5.prior.prior.prior._identity_state(display_game, display_away, display_home, visuals)
    away_stats = v5.prior.prior._team_stats_state(display_away, display_game, "away")
    home_stats = v5.prior.prior._team_stats_state(display_home, display_game, "home")
    ...
    st.markdown(v6._monster_matchup_hero(identity, away_stats, home_stats), unsafe_allow_html=True)
    st.markdown(v6._compact_game_strip(identity), unsafe_allow_html=True)
    st.markdown(v6._scoring_defense_summary(away_stats, home_stats), unsafe_allow_html=True)

    with st.container(border=True):
        st.markdown(
            '<div class="gt157-flowhead" data-testid="gt157-connected-flow"><b>CONNECTED GAME TOTAL FLOW</b><span>Steps 1–10 → Step 11 Distribution → Step 12 Final → Top-5. Deep evidence stays collapsed below, just like the cleaner Hit-page scan path.</span></div>',
            unsafe_allow_html=True,
        )
        v6._render_steps_1_10_rail(identity, away_evidence, home_evidence, display_game)
