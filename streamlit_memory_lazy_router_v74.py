"""KYRE Streamlit Router V74 — CFB O/U cache-stable fast route.

Step 3 of the College Football Over/Under performance work.

V74 preserves Router V73's direct CFB Over/Under fast route and advances only
that already-active route from Clean Page V32 to Clean Page V33. V33 keeps the
performance profiler active while stabilizing frozen analysis cache inputs
against display-only ``market_*`` timestamp/context churn.

Every non-active-CFB-O/U route delegates to certified Router V73 unchanged.
"""
...
import streamlit_memory_lazy_router_v73 as prior
import streamlit_memory_lazy_router_v1 as root

MODEL_VERSION = "KYRE STREAMLIT ROUTER V74 • CFB O/U CACHE-STABLE FAST ROUTE"
FROZEN_ROUTER = "streamlit_memory_lazy_router_v73"
CFB_SPORT_LABEL = prior.CFB_SPORT_LABEL
CFB_MARKETS = tuple(prior.CFB_MARKETS)
OVER_UNDER_MARKET = "Over/Under"
ACTIVE_PAGE = "cfb_over_under_clean_page_v33"
...
def _selectbox_v74(label: Any, options: Any, *args: Any, **kwargs: Any):
...
def _render_cfb_ou_direct(market: str) -> None:
    """Dispatch only active CFB Over/Under directly to Clean Page V33."""
    if (
        str(st.session_state.get("ks_sport_touch") or "") != CFB_SPORT_LABEL
