"""Regression checks for additive Moneyline Router V6."""
import types

import streamlit_memory_lazy_router_v6 as r


def test_moneyline_routes_directly_to_v168(monkeypatch):
    games = object()
    seen = {}
    ...
    r._render_mlb_v6_base("Moneyline")

    assert seen["module"] == "mlb_moneyline_hub_v168"
    assert seen["args"][0] is games
    assert "Moneyline" in seen["caption"]


def test_non_moneyline_uses_captured_frozen_v5_route(monkeypatch):
    seen = {}
    monkeypatch.setattr(r, "_BASE_MLB_ROUTE", lambda market: seen.setdefault("market", market))

    r._render_mlb_v6_base("Hits")

    assert seen["market"] == "Hits"


def test_render_app_temporarily_swaps_v5_hook_and_restores(monkeypatch):
    original = r.frozen_router._render_mlb_v5_base
    seen = {}

    def delegated_render():
        seen["hook"] = r.frozen_router._render_mlb_v5_base

    monkeypatch.setattr(r.frozen_router, "render_app", delegated_render)

    r.render_app()

    assert seen["hook"] is r._render_mlb_v6_base
    assert r.frozen_router._render_mlb_v5_base is original
