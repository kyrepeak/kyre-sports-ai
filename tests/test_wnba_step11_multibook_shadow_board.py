...
def _payload(
    ...
    *,
    line: float = 20.5,
    captured: str = "2026-08-28T06:19:45+00:00",
    over: int = -110,
    under: int = -110,
    stat: str = "points",
    player_id: int = PLAYER_ID,
...
def _bridge(provider: str, *, line: float = 20.5, over: int = -110, under: int = -110) -> dict:
    payload = _payload(provider, line=line, over=over, under=under)
    guards = {
...
def _fetcher(provider: str, *, line: float = 20.5, over: int = -110, under: int = -110):
    bridge = _bridge(provider, line=line, over=over, under=under)
...
def _run(**overrides):
    kwargs = {
        ...
        "provider_attempts": 3,
        "draftkings_fetcher": _fetcher(dk.PROVIDER, over=-105, under=-115),
        "fanduel_fetcher": _fetcher(fd.PROVIDER, over=-110, under=-110),
        "env": _env(),
    }
...
class Step11MultiBookShadowBoardTests(unittest.TestCase):
    ...
    def test_happy_path_builds_true_two_book_qualified_shadow_card(self):
        ...
        self.assertEqual(result["pipeline_result"]["refresh_cycle"]["snapshot_source"], "current_refresh")

    def test_different_lines_never_create_fake_consensus(self):
        result = _run(
            fanduel_fetcher=_fetcher(fd.PROVIDER, line=21.5, over=-110, under=-110),
        )
    ...
    def test_retryable_failure_then_success_is_bounded_without_sleep(self):
        ...
        fd_status = next(row for row in result["providers"] if row["provider"] == "FanDuel")
        self.assertEqual(fd_status["attempts_executed"], 2)
        self.assertEqual(fd_status["retryable_failures"], 1)
        self.assertTrue(fd_status["succeeded"])
        self.assertFalse(result["guardrails"]["scheduler_started"])

    def test_terminal_identity_error_is_never_hidden_as_outage(self):
        def fail(**kwargs):
    ...
    def test_wrong_fanduel_release_is_rejected(self):
        bad = _bridge(fd.PROVIDER)
        bad["release_id"] = "wrong"
    ...
    def test_provider_order_is_deterministic(self):
        ...
        self.assertEqual([row["provider"] for row in result["provider_refreshes"]], ["DraftKings", "FanDuel"])

    def test_exact_line_audit_lists_both_books(self):
        result = _run()
        group = result["market_audit"]["exact_line_multibook_groups"][0]
        self.assertEqual(group["line"], 20.5)
        self.assertEqual(group["sportsbooks"], ["DraftKings", "FanDuel"])
        self.assertEqual(group["sportsbook_count"], 2)
    ...
    def test_shadow_guardrails_allow_analysis_but_no_writes_or_activation(self):
        ...
        self.assertTrue(guards["cross_sportsbook_consensus_calculated_in_frozen_step9"])
        for key in (
            "authentication_used",
            "cookies_used",
    ...
    def test_no_new_public_step11d_fastapi_route_exists(self):
        paths = app.openapi()["paths"]
        self.assertFalse(any("step11" in path.casefold() or "shadow" in path.casefold() for path in paths))

    def test_shadow_hash_is_stable_across_generation_time(self):
        first = _run()
        second = _run()
        self.assertEqual(first["shadow_board_content_sha256"], second["shadow_board_content_sha256"])

    def test_tampered_step8_distribution_fails_frozen_step9_integrity(self):
        distribution = _step8_result()
        distribution["distributions"]["points"]["probability_mass"][0]["probability"] = 0.1
