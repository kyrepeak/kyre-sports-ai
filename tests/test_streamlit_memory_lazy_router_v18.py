"""Regression checks for additive MLB Hits Step 3 Router V18."""
import types

import streamlit_memory_lazy_router_v18 as r


def test_hits_routes_directly_to_v1318(monkeypatch):
    games = object()
    seen = {}
    ...
    r._render_mlb_v18_base("1+ Hit")

    assert seen["module"] == "mlb_hit_hub_v1318"
    assert seen["install"] == "1+ Hit"
    assert seen["args"][0] is games
    assert "1+ Hit" in seen["caption"]


def test_non_hits_routes_through_frozen_router_v17(monkeypatch):
    seen = []
    monkeypatch.setattr(r, "_BASE_MLB_ROUTE", lambda market: seen.append(market))
    r._render_mlb_v18_base("Moneyline")
    r._render_mlb_v18_base("Matchup Explorer")
    assert seen == ["Moneyline", "Matchup Explorer"]


def test_render_app_temporarily_swaps_v17_hook_and_restores(monkeypatch):
    original = r.frozen_router._render_mlb_v17_base
    seen = {}

    def delegated_render():
        seen["hook"] = r.frozen_router._render_mlb_v17_base

    monkeypatch.setattr(r.frozen_router, "render_app", delegated_render)
    r.render_app()

    assert seen["hook"] is r._render_mlb_v18_base
    assert r.frozen_router._render_mlb_v17_base is original
