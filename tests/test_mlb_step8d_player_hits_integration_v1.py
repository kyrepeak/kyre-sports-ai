...
def _prop(game=1001, player=2001, market=PLAYER_HITS, name="FanDuel Display"):
    return {
...
def test_exact_id_attachment_ignores_player_name_completely():
    ...
    integration = build_player_hits_integration(results, state)
...
def test_enrichment_preserves_every_preexisting_hit_model_field_and_value():
    state = _fresh_state(_prop())
    original = _result()
    integration = build_player_hits_integration([original], state)
    enriched = enrich_player_hit_results([original], integration)
    ...
    assert enriched[0]["screen_p1"] == original["screen_p1"]


def test_stale_step8a_state_fails_closed_to_preexisting_hit_model():
    payload = {
        "data_type": "mlb_player_prop_api_response_v1",
        "schema_version": 1,
        "source": "FanDuel",
    ...
    original = [_result()]
    integration = build_player_hits_integration(original, stale)
    ...
    assert enrich_player_hit_results(original, integration) == original
...
def test_same_player_name_with_wrong_official_id_never_matches():
    ...
    integration = build_player_hits_integration(
...
def test_pitcher_strikeout_context_never_attaches_to_player_hits():
    state = _fresh_state(_prop(market=PITCHER_STRIKEOUTS))
    integration = build_player_hits_integration([_result()], state)
    assert integration["attached_count"] == 0
    assert integration["unmatched_count"] == 1
    assert integration["market_type"] == PLAYER_HITS
...
def test_duplicate_exact_player_hits_result_identity_globally_fails_closed():
    state = _fresh_state(_prop())
    ...
    assert integration["attached_count"] == 0
    assert "duplicate_exact_player_hits_result_identity" in integration["failures"]
    assert enrich_player_hit_results(results, integration) == results
...
def test_inactive_integration_returns_deep_copied_results_without_attachment():
    ...
    enriched = enrich_player_hit_results(original, fallback)
...
def test_build_and_enrich_do_not_mutate_inputs():
    ...
    integration = build_player_hits_integration(results, api_state)
    integration_before = deepcopy(integration)
    _ = enrich_player_hit_results(results, integration)
...
def test_attachment_is_display_only_and_has_zero_math_or_ranking_impact_flags():
    integration = build_player_hits_integration([_result()], _fresh_state(_prop()))
    context = integration["attachments_by_result_index"][0]
...
def test_multiple_exact_hitters_attach_independently_by_ids_only():
    state = _fresh_state(
    ...
    integration = build_player_hits_integration(results, state)
    enriched = enrich_player_hit_results(results, integration)

    assert integration["attached_count"] == 2
    assert integration["unmatched_count"] == 0
    assert enriched[0][ATTACHMENT_KEY]["line"] == 0.5
    assert enriched[1][ATTACHMENT_KEY]["line"] == 1.5
    assert enriched[1][ATTACHMENT_KEY]["over_odds"] == 145
    assert enriched[1][ATTACHMENT_KEY]["under_odds"] == -180
...
def test_boolean_ids_are_not_accepted_as_integer_identity():
    state = _fresh_state(_prop())
    integration = build_player_hits_integration([_result(game=True, player=2001)], state)
    assert integration["invalid_result_count"] == 1
    assert integration["attached_count"] == 0
