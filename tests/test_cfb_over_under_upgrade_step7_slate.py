"""Regression checks for CFB O/U Upgrade Step 7 slate V6."""
from __future__ import annotations

import cfb_over_under_slate_v6 as slate
...
def test_analyze_game_runs_step6_then_third_down_then_final(monkeypatch):
    game = _game()
    seen = []
    ...
    monkeypatch.setattr(
        slate.frozen,
        "analyze_game",
        lambda g, d, line: (seen.append("step6") or _base(g)),
    )
    monkeypatch.setattr(
        slate.third_down_engine,
        "build_third_down_engine",
        lambda g, a, h: (
            seen.append("td_build")
            or {"model_ready": True, "coverage": 1.0}
        ),
    )

    def apply(base_raw, engine):
        seen.append("td_apply")
        out = dict(base_raw)
        out["upgrade_step7_applied"] = True
        out["projected_total"] = 50.4
        return out

    monkeypatch.setattr(slate.third_down_engine, "apply_to_raw", apply)
    monkeypatch.setattr(
        slate.final_model,
        "synthesize",
        lambda g, raw: (
    ...
    assert seen == ["step6", "td_build", "td_apply", "final"]
    assert out["step6_raw"]["projected_total"] == 50.0
    assert out["raw"]["upgrade_step7_applied"] is True
    assert out["final"]["projected_total"] == 50.4
    assert out["analysis_line_third_down_weight"] == 0.0


def test_scan_slate_reports_third_down_engine(monkeypatch):
    games = [_game("g1"), _game("g2")]

    def fake_analyze(game, day, line):
        return {
            "game": dict(game),
            "raw": {"ready": True, "upgrade_step7_applied": True},
            "final": {
                "ready": True,
                "rank_eligible": game["identity_key"] == "g1",
            },
            "third_down_engine": {"model_ready": True},
        }
    ...
    assert len(rows) == 2
    assert diag["third_down_engine_ready"] == 2
    assert diag["third_down_engine_applied"] == 2
    assert diag["qualified_plays"] == 1
    assert diag["analysis_line_third_down_weight"] == 0.0
