...
def test_step5_uses_only_source_proven_espn_explosive_fields():
...
def test_step5_extracts_explosive_rate_from_exact_espn_fields():
    ...
    payload = {
        "results": {
            "stats": {
                "categories": [
                    {"name": "rushing", "stats": [
                        {"name": "totalOffensivePlays", "value": 200},
    ...
    assert metrics["offensive_plays"] == 200.0
    assert metrics["explosive_plays_per_100"] == 7.0


def test_step5_explosive_labels_are_simple_and_deterministic():
    module = _load_context_module()
    assert module.classify_explosive_scoring(7.0) == "FAVORABLE"
    assert module.classify_explosive_scoring(5.5) == "MEDIUM"
    assert module.classify_explosive_scoring(4.0) == "TOUGH"
...
def test_step5_same_matchup_card_grows_downward_from_v4():
    ...
    assert "20+ RUSH" in source
    assert "20+ RECEIVE" in source
    assert "EXPLOSIVE / 100 PLAYS" in source
