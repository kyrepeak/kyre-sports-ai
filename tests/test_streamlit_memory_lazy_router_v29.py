"""Regression checks for additive CFB Router V29."""
...
def test_cfb_moneyline_routes_to_scoreboard_complete_hub(monkeypatch):
    seen = {}
    monkeypatch.setattr(
        router.st,
        "session_state",
...
def test_other_markets_delegate_to_frozen_v28(monkeypatch):
    seen = []
    monkeypatch.setattr(
        router.st,
        "session_state",
...
def test_non_cfb_delegates_to_frozen_v28(monkeypatch):
    seen = []
