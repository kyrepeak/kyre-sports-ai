...
import cfb_over_under_slate_v4 as frozen

MODEL_VERSION = "CFB OVER/UNDER SLATE V5 • UPGRADE STEP 6 RED ZONE"
FROZEN_SLATE = "cfb_over_under_slate_v4"
...
def analyze_game(
    ...
    raw = red_zone_engine.apply_to_raw(base_raw, engine)
...
def scan_slate(
    ...
    engine_ready = sum(
        bool((row.get("red_zone_engine") or {}).get("model_ready"))
        for row in rows
    )
    engine_applied = sum(
        bool((row.get("raw") or {}).get("upgrade_step6_applied"))
        for row in rows
    )
    ...
    return rows, {
        ...
        "red_zone_engine_ready": engine_ready,
        "red_zone_engine_applied": engine_applied,
