"""WNBA Daily Picks — Moneyline read-only connector V1.

Reads only completed same-session WNBA Moneyline V1.5 Step-8 grading state.
It never imports/runs the Moneyline production model, refreshes sportsbook data,
reruns simulations, changes probabilities, or writes source-model state.

A completed Step-8 PASS with zero QUALIFIED Moneylines is a healthy connected
...
MODEL_VERSION = "WNBA DAILY PICKS MONEYLINE CONNECTOR V1 • READ ONLY"
...
def _source(day: Any):
    target = _day(day)
    saved_day = _day(st.session_state.get("wnba_moneyline_v15_day"))
    grading_ready = bool(st.session_state.get("wnba_moneyline_v15_grading_ready", False))
    final = _frame(st.session_state.get("wnba_moneyline_v15_final_card"))
    grade_meta_raw = st.session_state.get("wnba_moneyline_v15_grade_meta")
    grade_meta = dict(grade_meta_raw) if isinstance(grade_meta_raw, dict) else {}
    mc_day = _day(st.session_state.get("wnba_moneyline_v14_mc_day"))
    mc = _frame(st.session_state.get("wnba_moneyline_v14_mc_detail"))
    mc_meta_raw = st.session_state.get("wnba_moneyline_v14_mc_meta")
...
def status(day: Any) -> dict[str, Any]:
    ...
    base = {
        ...
        "state": "⏳ NOT RUN",
        "connected": False,
        "source": "WNBA Moneyline V1.5 Step 8",
        "rows": 0,
        "production_picks": 0,
        "qualified": 0,
        "monitor": 0,
        ...
        "final_ready": 0,
        "ran_at": "—",
        "detail": "No completed same-day Moneyline Step-8 grading payload is present in this session.",
    }
    if not target:
        return base
    ...
    qualified = final.loc[final.get("grade", pd.Series("", index=final.index)).astype(str).str.upper().eq("QUALIFIED")].copy() if not final.empty else pd.DataFrame()
    monitor = int(final.get("grade", pd.Series(dtype=str)).astype(str).str.upper().eq("MONITOR").sum()) if not final.empty else 0
    no_play = int(final.get("grade", pd.Series(dtype=str)).astype(str).str.upper().eq("NO PLAY").sum()) if not final.empty else 0
    blocked = int(final.get("grade", pd.Series(dtype=str)).astype(str).str.upper().eq("BLOCKED").sum()) if not final.empty else 0
    ...
    base.update({
        ...
        "detail": (
            f"Read-only Moneyline Step-8 PASS • {len(qualified)} qualified game Moneyline(s) • {completed:,} game-level simulations"
            if connected else "Moneyline Step-8 output is missing or failed read-only validation."
        ),
    })
    return base
...
def preview_rows(day: Any, limit: int = 20) -> pd.DataFrame:
    ...
    qualified = final.loc[final.get("grade", pd.Series("", index=final.index)).astype(str).str.upper().eq("QUALIFIED")].copy()
    records = []
    for _, r in qualified.iterrows():
        ...
        records.append({
            ...
            "Projection": _num(r.get("mc_win_prob")),
            ...
            "Source": "Moneyline Step-8 5M final grading",
        })
...
def final_guard_proof(day: Any) -> pd.DataFrame:
    target, saved_day, grading_ready, final, grade_meta, mc_day, mc, mc_meta = _source(day)
    cols = [
        "Slate day", "Team", "Opponent", "Book", "Posted odds", "Tip ET proof",
        "Quote timestamp proof", "Quote age seconds proof", "Run timestamp proof",
