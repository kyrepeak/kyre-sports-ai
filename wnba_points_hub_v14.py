"""WNBA Points V1.4 — clean page using ET-reconciled WNBA schedule V2.5."""
...
import wnba_points_v11 as points
import wnba_schedule_v25 as schedule

MODEL_VERSION = "WNBA POINTS V1.4 • ET-RECONCILED CLEAN SLATE"
PRA_FROZEN_BRANCH = ui.PRA_FROZEN_BRANCH
PRA_FROZEN_COMMIT = ui.PRA_FROZEN_COMMIT
MLB_FROZEN_BRANCH = ui.MLB_FROZEN_BRANCH

# Patch only this isolated Points page's module globals. PRA/MLB are untouched.
ui.points = points
...
<div style="border:1px solid #2f6381;background:linear-gradient(145deg,#091c2d,#071421);border-radius:20px;padding:16px;margin:8px 0 14px">
  ...
  <div style="font-size:30px;font-weight:1000;color:white;margin-top:5px">🏀 WNBA Points Command Center — V1.4</div>
  <div style="font-size:12px;color:#93aabd;line-height:1.55;margin-top:7px">Eastern-time slate verification + cross-source reconciliation. Every verified game stays visible; only eligible upcoming games feed the Points model. PRA V3.2.1 and MLB V2.1.7 remain frozen.</div>
</div>
""", unsafe_allow_html=True)
    ...
    if state in {"VERIFIED", "VERIFIED_SINGLE_SOURCE", "VERIFIED_OFF_DAY"}:
        st.success(f"✅ Verified WNBA slate • {day} • all {slate['total']} game(s) shown below • slate date = Eastern Time")
    else:
    ...
    if counts:
        ...
        st.caption(f"🧭 Schedule cross-check — {summary}")


def render_wnba_points_hub(section_header=None, status_info=None, team_logo=None, h=None):
    st.caption("🏀 WNBA Points V1.4 • ET-reconciled schedule V2.5 • PRA V3.2.1 frozen • SportsGameOdds WNBA • MLB V2.1.7 frozen")
    ...
    with st.expander("🧊 Freeze / isolation status", expanded=False):
        ...
        st.write("Points reads the corrected WNBA V2.5 schedule only inside the isolated Points stack. Frozen PRA and MLB production modules remain unchanged.")
