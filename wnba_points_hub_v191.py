"""WNBA Points visual compatibility layer.

UI-only compatibility wrapper over validated Points V1.9 production math.
This module is intentionally kept at the legacy v191 import path because deeper
inherited shells may still call it directly. It now renders the same V1.9.3
visual command center and WNBA matchup-logo cards used by the current Points
page. Projection, SportsGameOdds matching, calibration, Monte Carlo,
persistence, and decision math remain unchanged.
...
def _clean_header(day, slate):
    """Presentation-only V1.9.3 command center over the validated V1.9 engine."""
    _visual_css()
    ...
    if isinstance(current, pd.DataFrame) and not current.empty:
        ...
        distributions = int(current[needed].drop_duplicates().shape[0]) if len(needed) == 3 else int(len(current))
...
<div class="kyre-wnba-hero">
  ...
  <div class="kyre-wnba-sub">Verified four-game Eastern slate • current rosters • rotation-aware L3/L5 minutes • scoring role/usage • recent + season form • opponent pace + L10 defense • Guard/Wing/Big matchup • empirical variance • exact SportsGameOdds Points markets. Production projection and Monte Carlo math remain unchanged.</div>
</div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="kyre-engine-note">⚙️ <b>Production engine room below:</b> roster, rotation minutes, history, matchup, Monte Carlo and calibration checks remain visible for auditability. The visual layer above does not alter any projection.</div>',
        unsafe_allow_html=True,
    )
...
def render_wnba_points_hub(section_header=None, status_info=None, team_logo=None, h=None):
    # The inherited render chain owns validated data/model hooks. We replace only
    # its header renderer and legacy version chrome.
    ...
    original_caption(
        "🏀 WNBA Points V1.9.3 • visual command center active • production math V1.9 • PRA V3.2.1 frozen • MLB V2.1.7 frozen"
    )
