"""OFF-only live certification of Step 7G first-party Step-4F advanced context.
...
shot context to remain pass, zero blockers, and projection start permission.
...
def main() -> int:
    ...
    for label, advanced in (("team", team_advanced), ("opponent", opponent_advanced)):
        for field in ("estimated_offensive_rating", "estimated_defensive_rating", "estimated_net_rating", "estimated_pace"):
    ...
    # integration installs candidate Step-4F seams before router binding.
    ...
    if status.get("candidate_scope", {}).get("advanced_context") is not True:
        raise RuntimeError("Step 4F candidate scope is not declared.")
    if status.get("certified_scope", {}).get("advanced_context") is not False:
        raise RuntimeError("Step 4F was marked certified before the live cert completed.")
    ...
    if body.get("can_start_projection") is not True:
        raise RuntimeError("Step 4X did not allow projection start after Step 4F integration.")
    ...
    report = {
        "data_type": "wnba_step7g_step4f_first_party_advanced_context_cert_v1",
        ...
        "certification_result": "STEP4F_FIRST_PARTY_ADVANCED_CONTEXT_LIVE_CERTIFIED",
        "safety": {
