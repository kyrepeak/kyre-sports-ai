"""Step 8E exact-ID integration for MLB hits+runs+RBI sportsbook context.

This module is a pure, read-only adapter between the frozen Step 8A player-prop
contract and already-produced MLB result objects. FanDuel line/price metadata is
attached only after a result has been identified by exact official MLB game ID
+ official MLB player ID. Display names never participate in identity and
sportsbook data never changes model output.
"""
...
DATA_TYPE = "mlb_step8e_hits_runs_rbi_integration_v1"
SCHEMA_VERSION = 1
INTEGRATED = "HITS_RUNS_RBI_API_CONTEXT_ATTACHED"
FALLBACK = "HITS_RUNS_RBI_PREEXISTING_MODEL_FALLBACK"
ATTACHMENT_KEY = "step8e_hits_runs_rbi_context"
...
def _result_identity(row: Mapping[str, Any]) -> tuple[int, int, str] | None:
    ...
    if game_id is None or player_id is None:
        return None
    return (game_id, player_id, HITS_RUNS_RBI)
...
def build_hits_runs_rbi_integration(
    player_results: Sequence[Mapping[str, Any]] | None,
    api_state: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Build display-only exact-ID H+R+RBI attachments without modifying model rows."""
    original_rows = list(player_results or [])
    ...
    for index, raw_row in enumerate(rows):
        ...
        if identity in seen:
            ...
            invalid.append(
                {
                    "index": index,
                    "reason": "duplicate_exact_hits_runs_rbi_result_identity",
    ...
    if duplicate:
        failures.append("duplicate_exact_hits_runs_rbi_result_identity")
    ...
    if not failures:
        for index, identity in identities.items():
                ...
                unmatched.append(
                    {
                        ...
                        "official_player_id": player_id,
                        "market_type": market_type,
                        "reason": "exact_hits_runs_rbi_context_unavailable",
                    }
                )
                continue
            if context.get("data_type") != STEP8A_CONTEXT_DATA_TYPE:
                ...
                continue
            attachments[index] = {
                "data_type": "mlb_step8e_hits_runs_rbi_attachment_v1",
    ...
    return {
        ...
        "preexisting_hits_runs_rbi_model_preserved": True,
        "model_math_impact": False,
        "projection_impact": False,
        "simulation_impact": False,
        "probability_impact": False,
...
def enrich_hits_runs_rbi_results(
    player_results: Sequence[Mapping[str, Any]] | None,
    integration_state: Mapping[str, Any] | None,
) -> list[dict[str, Any]]:
    """Return deep-copied result rows with display-only H+R+RBI context where proven."""
    rows = deepcopy(list(player_results or []))
    ...
    for raw_index, raw_context in attachments.items():
        ...
        if (
            ...
            or context.get("market_type") != HITS_RUNS_RBI
