"""Kyre Sports API — NFL Receiving Yards verified post-projection market endpoint.

The endpoint exposes only fresh exact-ID FanDuel market context. It does not
...
router = APIRouter(
    prefix="/api/v1/nfl/receiving-yards/market",
    tags=["nfl-receiving-yards-market"],
)
...
@router.get("/status")
def receiving_yards_market_status():
    return {"status": "ready", "service": "kyre-sports-api", **CONTRACT}


@router.get("")
def receiving_yards_market(
    event_id: str = Query(
        ...,
        min_length=1,
        max_length=32,
    ...
    try:
        payload = collect_fanduel_nfl_receiving_yards_hosted(event_id)
    except NFLReceivingYardsMarketCollectorError as exc:
        raise HTTPException(
            status_code=503,
            detail=f"NFL Receiving Yards market unavailable: {str(exc)[:240]}",
        ) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=503,
            detail=f"NFL Receiving Yards market unavailable: {type(exc).__name__}",
        ) from exc
    ...
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise HTTPException(
            status_code=503,
            detail="NFL Receiving Yards market schema contract mismatch",
        )
    if str(payload.get("official_event_id") or "") != event_id:
        raise HTTPException(
            status_code=503,
            detail="NFL Receiving Yards market official event identity mismatch",
        )
    if (
        ...
        raise HTTPException(
            status_code=503,
            detail="NFL Receiving Yards Step 8 market safety contract failed closed",
        )
    ...
    for row in payload.get("props") or []:
        ...
        if (
            ...
            or position not in {"WR", "TE", "RB", "FB"}
            or str(row.get("market_type") or "").strip().lower() != "receiving_yards"
            or str(row.get("sportsbook") or "").strip().lower() != "fanduel"
            or str(row.get("line_status") or "").strip().lower() != "active"
        ):
            raise HTTPException(
                status_code=503,
                detail="NFL Receiving Yards market row identity contract failed closed",
            )
