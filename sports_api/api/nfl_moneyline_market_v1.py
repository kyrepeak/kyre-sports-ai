"""Kyre Sports API — exact-ID NFL pregame Moneyline market endpoint.

The endpoint is transport/market context only. It returns fresh sportsbook rows
for one official ESPN NFL event and cannot modify the frozen Moneyline model,
Monte Carlo, probability, grading, staking, or wager behavior.
"""
...
from sports_api.collectors.nfl_fanduel_moneyline_v1 import (
    NFLMoneylineCollectorError,
    SCHEMA_VERSION,
    collect_fanduel_nfl_moneyline,
)

router = APIRouter(prefix="/api/v1/nfl/moneyline/market", tags=["nfl-moneyline-market"])
...
def _validate_market_payload(payload: dict[str, Any], event_id: str) -> dict[str, Any]:
    """Re-run identity, price and 0%-projection contracts for every response."""
    if not isinstance(payload, dict):
        raise HTTPException(status_code=503, detail="NFL Moneyline market payload is invalid")
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise HTTPException(status_code=503, detail="NFL Moneyline market schema contract mismatch")
    if _text(payload.get("official_event_id")) != event_id:
        raise HTTPException(status_code=503, detail="NFL Moneyline official event identity mismatch")
    if not _aware_iso(payload.get("captured_at_utc")):
        raise HTTPException(status_code=503, detail="NFL Moneyline capture timestamp is invalid")
    ...
    if (
        ...
        raise HTTPException(status_code=503, detail="NFL Moneyline market safety contract failed closed")

    books = payload.get("books") or []
    if not isinstance(books, list):
        raise HTTPException(status_code=503, detail="NFL Moneyline books payload is invalid")
    seen: set[str] = set()
    validated_books: list[dict[str, Any]] = []
    for raw in books:
        if not isinstance(raw, dict):
            raise HTTPException(status_code=503, detail="NFL Moneyline book row is invalid")
        sportsbook = _text(raw.get("sportsbook"))
        key = sportsbook.casefold()
        away_ml = _american(raw.get("away_ml"))
        home_ml = _american(raw.get("home_ml"))
        if (
            ...
            raise HTTPException(status_code=503, detail="NFL Moneyline book row contract failed closed")
        seen.add(key)
        row = dict(raw)
        row["away_ml"] = away_ml
        row["home_ml"] = home_ml
    ...
    if payload.get("market_available") is True and not validated_books:
        raise HTTPException(status_code=503, detail="NFL Moneyline marked available without a valid book pair")
    if payload.get("ready") is True and not validated_books:
        raise HTTPException(status_code=503, detail="NFL Moneyline marked ready without a valid book pair")
...
def _collector_identity() -> int:
    return id(collect_fanduel_nfl_moneyline)
...
def _collect_or_reuse_market(event_id: str) -> dict[str, Any]:
    ...
    with _event_lock(event_id):
        ...
        if cached is not None:
            return _validate_market_payload(cached, event_id)
        try:
            payload = collect_fanduel_nfl_moneyline(event_id)
        except NFLMoneylineCollectorError as exc:
            raise HTTPException(
                status_code=503,
                detail=f"NFL Moneyline market unavailable: {str(exc)[:240]}",
            ) from exc
        except Exception as exc:
            raise HTTPException(
                status_code=503,
                detail=f"NFL Moneyline market unavailable: {type(exc).__name__}",
            ) from exc
