"""FastAPI transport for WNBA Step 5L verified daily-slate Top-5 assembly."""
...
class DailySlatePropLineInput(BaseModel):
    player_id: int
    stat: str
    line: float
    sportsbook_quotes: list[dict[str, Any]] | None = None


class DailySlateTopFiveInput(BaseModel):
    prop_lines: list[DailySlatePropLineInput]


def _raise_api_error(exc: Exception) -> None:
    if isinstance(exc, (ValueError, WNBADailySlateTopFiveModelInputError, WNBAPlayerPropBoardModelInputError)):
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    if isinstance(exc, (WNBADailySlateTopFiveNotReadyError, WNBAPlayerPropBoardNotReadyError)):
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    if isinstance(exc, (WNBADailySlateTopFiveUpstreamError, WNBAPlayerPropBoardUpstreamError)):
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    raise exc


@router.post("/rankings/player-props/daily-top-five")
def create_daily_slate_top_five_board(
    payload: DailySlateTopFiveInput,
    date: str | None = Query(
        default=None,
    ...
    max_snapshot_age_minutes: int = Query(
        default=DEFAULT_MAX_SNAPSHOT_AGE_MINUTES, ge=1, le=1440
    ),
    ...
    max_market_age_minutes: int = Query(
        default=DEFAULT_MAX_MARKET_AGE_MINUTES, ge=1, le=1440
    ),
    ...
    try:
        return build_daily_slate_top_five(
            [line.model_dump() for line in payload.prop_lines],
            date=date,
            season=season,
