...
from sports_api.wnba_baseline_projection import (
    ...
    WNBABaselineProjectionNotFoundError,
    WNBABaselineProjectionNotReadyError,
    WNBABaselineProjectionUpstreamError,
    get_player_game_baseline_projection,
)
...
@router.get("/games/{game_id}/players/{player_id}/baseline-projection")
def player_game_baseline_projection(
    game_id: str,
    player_id: int,
    ...
    last_n_games: int = Query(
        ...
        ge=1,
        le=20,
        description="Recent observed window used for official rotation and role-rate inputs.",
    ),
    require_current_availability: bool = Query(
        default=True,
        description="When true, Step 4X requires current roster/injury evidence before 5A may run.",
    ),
    ...
    try:
        return get_player_game_baseline_projection(
            player_id,
            game_id,
            season,
    ...
    except WNBABaselineProjectionNotFoundError as exc:
