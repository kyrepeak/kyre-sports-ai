"""Read-only FanDuel NFL Moneyline collector for Kyre Sports API.
...
class NFLMoneylineCollectorError(RuntimeError):
    """The NFL Moneyline market could not be proven safely."""
...
def _role_runner(market: Mapping[str, Any], role: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise NFLMoneylineCollectorError(
            f"Moneyline expected exactly one active {role} runner, found {len(matches)}"
        )
    return matches[0]
...
def _american_odds(runner: Mapping[str, Any]) -> int:
    ...
    try:
        ...
        raise NFLMoneylineCollectorError("Moneyline runner has no valid American odds") from exc
    if price == 0 or abs(price) < 100:
        raise NFLMoneylineCollectorError("Moneyline American odds are outside the supported contract")
    return price


def _moneyline_market(event_page: Mapping[str, Any], provider_event_id: str) -> dict[str, Any]:
    attachments = event_page.get("attachments") if isinstance(event_page, Mapping) else {}
    if not isinstance(attachments, Mapping):
        raise NFLMoneylineCollectorError("FanDuel NFL event page has no attachments object")

    candidates: list[dict[str, Any]] = []
    for market in _rows(attachments.get("markets")):
        if _market_name(market) != MARKET_NAME or not _market_is_open_pregame(market):
            continue
    ...
    if not candidates:
        raise NFLMoneylineCollectorError("no open pregame FanDuel Moneyline market was found")
...
def normalize_moneyline_market(
    ...
    *,
    ...
    if not event_id.isdigit():
        raise NFLMoneylineCollectorError("official ESPN NFL event ID is invalid")
        ...
        "books": [
            {
                ...
                "away_ml": _american_odds(away),
                "home_ml": _american_odds(home),
...
def collect_fanduel_nfl_moneyline(
    ...
    *,
    ...
    """Collect one exact-ID FanDuel NFL pregame Moneyline snapshot."""
    event_id = _text(official_event_id)
    if not event_id.isdigit():
        raise NFLMoneylineCollectorError("official ESPN event_id must be numeric")
    ...
    try:
        ...
        if not provider_event_id:
            raise NFLMoneylineCollectorError("reconciled FanDuel event ID is missing")
        event_page = (event_page_fetcher or fetch_fanduel_event_page)(provider_event_id)
        market = _moneyline_market(event_page, provider_event_id)
        return normalize_moneyline_market(
            market,
            official_event=official,
            provider_event=provider_event,
            captured_at_utc=now,
        )
    except NFLMoneylineCollectorError:
        raise
    except NFLPassingYardsCollectorError as exc:
        raise NFLMoneylineCollectorError(str(exc)) from exc
    except Exception as exc:
        raise NFLMoneylineCollectorError(
            f"NFL Moneyline collection failed: {type(exc).__name__}"
        ) from exc
