"""Read-only FanDuel NFL Spread collector for Kyre Sports API.

Reuses the certified NFL ESPN <-> FanDuel exact-event reconciliation and
extracts only the canonical open pregame two-way point spread market. No model
output is read or modified here.
"""
...
SPORTSBOOK = "FanDuel"
CANONICAL_MARKET_NAMES = {"spread", "point spread"}
MAX_ABS_NFL_SPREAD = 100.0


class NFLSpreadCollectorError(RuntimeError):
    """The NFL Spread market could not be proven safely."""
...
def _market_id(market: Mapping[str, Any]) -> str:
    return _text(market.get("marketId") or market.get("_attachment_key"))
...
def _role_runner(market: Mapping[str, Any], role: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise NFLSpreadCollectorError(
            f"Spread expected exactly one active {role} runner, found {len(matches)}"
        )
    return matches[0]


def _spread(runner: Mapping[str, Any]) -> float:
    try:
        value = float(runner.get("handicap"))
    except (TypeError, ValueError) as exc:
        raise NFLSpreadCollectorError("Spread runner has no valid handicap") from exc
    if not math.isfinite(value) or abs(value) > MAX_ABS_NFL_SPREAD:
        raise NFLSpreadCollectorError("Spread runner handicap is outside the supported NFL range")
    return value
...
def _american_odds(runner: Mapping[str, Any]) -> int:
    ...
    display = (win or {}).get("americanDisplayOdds") if isinstance((win or {}).get("americanDisplayOdds"), Mapping) else {}
    ...
    try:
        ...
        raise NFLSpreadCollectorError("Spread runner has no valid American odds") from exc
    if price == 0 or abs(price) < 100:
        raise NFLSpreadCollectorError("Spread American odds are outside the supported contract")
    return price


def _spread_market(event_page: Mapping[str, Any], provider_event_id: str) -> dict[str, Any]:
    attachments = event_page.get("attachments") if isinstance(event_page, Mapping) else {}
    if not isinstance(attachments, Mapping):
        raise NFLSpreadCollectorError("FanDuel NFL event page has no attachments object")

    candidates: list[dict[str, Any]] = []
    for market in _rows(attachments.get("markets")):
        name = _market_name(market).casefold()
        if name not in CANONICAL_MARKET_NAMES or not _market_is_open_pregame(market):
            continue
    ...
    if not candidates:
        raise NFLSpreadCollectorError("no open pregame FanDuel Spread market was found")

    candidates.sort(
        key=lambda row: (
            0 if _market_name(row).casefold() == "spread" else 1,
            int(row.get("sortPriority") or 10_000),
            _market_id(row),
        )
    )
    return candidates[0]
...
def normalize_spread_market(
    ...
    *,
    ...
    if not _market_is_open_pregame(market):
        raise NFLSpreadCollectorError("Spread market is not open pregame")

    event_id = _text(official_event.get("event_id"))
    if not event_id.isdigit():
        raise NFLSpreadCollectorError("official ESPN NFL event ID is invalid")

    away = _role_runner(market, "AWAY")
    home = _role_runner(market, "HOME")
    away_spread = _spread(away)
    home_spread = _spread(home)
    if not math.isclose(away_spread + home_spread, 0.0, abs_tol=1e-9):
        raise NFLSpreadCollectorError(
            f"Spread pair mismatch: away={away_spread} home={home_spread}"
        )
        ...
        "books": [
            {
                ...
                "away_price": _american_odds(away),
                "home_price": _american_odds(home),
...
def collect_fanduel_nfl_spread(
    ...
    *,
    ...
    """Collect one exact-ID FanDuel NFL pregame Spread snapshot."""
    event_id = _text(official_event_id)
    if not event_id.isdigit():
        raise NFLSpreadCollectorError("official ESPN event_id must be numeric")
    ...
    try:
        ...
        if not provider_event_id:
            raise NFLSpreadCollectorError("reconciled FanDuel event ID is missing")
        event_page = (event_page_fetcher or fetch_fanduel_event_page)(provider_event_id)
        market = _spread_market(event_page, provider_event_id)
        return normalize_spread_market(
            market,
            official_event=official,
            provider_event=provider_event,
            captured_at_utc=now,
        )
    except NFLSpreadCollectorError:
        raise
    except NFLPassingYardsCollectorError as exc:
        raise NFLSpreadCollectorError(str(exc)) from exc
    except Exception as exc:
        raise NFLSpreadCollectorError(
            f"NFL Spread collection failed: {type(exc).__name__}"
        ) from exc
