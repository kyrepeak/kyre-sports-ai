"""WNBA Daily Picks V2 — Step 2 PRA read-only connector.

Step 1's isolated page is preserved. Step 2 adds exactly one passive connector:
PRA. The page reads already-completed same-day PRA Step-8 payloads from Streamlit
session state and displays their health/counts. It never imports PRA production
modules, launches simulations, restores snapshots, refreshes injuries, requests
sportsbook data, regrades rows, changes projections, or writes to PRA state.
Points and Rebounds remain paused and disconnected.
"""
...
MODEL_VERSION = "WNBA DAILY PICKS V2 • STEP 2 PRA READ ONLY"
...
def render_wnba_daily_picks_hub(section_header=None, status_info=None, team_logo=None, h=None):
    ...
    st.markdown(
        ...
        <div class="ks-dp-hero">
          <div class="ks-dp-kicker">KYRE SPORTS AI • WNBA DAILY PICKS • STEP 2</div>
          <div class="ks-dp-title">🏆 WNBA Daily Picks Command Center</div>
          <div class="ks-dp-sub">
            Independent cross-market workspace. Step 2 adds a PRA read-only connector only.
            Daily Picks may inspect an already-completed same-day PRA payload, but it cannot run, restore,
            regrade, refresh, modify or write back to PRA. Points and Rebounds remain paused.
          </div>
          <span class="ks-dp-chip">📅 ET slate {slate_day}</span>
          <span class="ks-dp-chip">🔌 1 read-only connector</span>
          <span class="ks-dp-chip">🚫 zero simulations launched here</span>
          <span class="ks-dp-chip">🔒 zero PRA writes</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.success(
        "✅ STEP 2 ACTIVE • PRA is connected read-only. Daily Picks can inspect completed PRA output without controlling the PRA model."
    )

    st.markdown("### 🧩 Market Feed Status")
    st.caption("Only PRA is inspected in Step 2. Points and Rebounds remain deliberately disconnected.")
    ...
    st.caption(
        f"Slate {pra.get('day') or slate_day} • source {pra.get('source') or 'NONE'} • "
        f"5M ran at {pra.get('ran_at') or '—'} • 10M ran at {pra.get('final_ran_at') or '—'}"
    )

    if pra.get("connected"):
        st.success("✅ PRA READ-ONLY CHECK PASSED • A completed, converged same-day PRA 5M payload is visible to Daily Picks.")
    else:
        st.info("⏳ PRA NOT RUN / NOT LOADED • Daily Picks remains healthy. Run PRA only from the PRA page if you want a PRA payload to exist.")

    with st.expander("📋 PRA saved-output preview — display only", expanded=False):
        preview = _preview_display(pra_feed.preview_rows(slate_day, limit=12))
        if preview.empty:
            st.info("No same-day PRA rows are currently loaded in this Streamlit session.")
        else:
            st.dataframe(preview, use_container_width=True, hide_index=True)
        st.caption("Preview only • no ranking • no best-pick selection • no Daily Picks qualification performed in Step 2.")

    st.markdown("### 🏆 Top 5 WNBA Picks")
    st.info(
        "Step 2 intentionally does not populate the Top 5. PRA visibility is being verified first; cross-market ranking comes later."
    )

    with st.expander("🛡️ Step-2 isolation diagnostics", expanded=False):
        st.write("• Connected production feeds: PRA read-only only")
        st.write("• PRA production-module imports by Daily Picks connector: 0")
        st.write("• Sportsbook requests launched by Daily Picks: 0")
        st.write("• Monte Carlo runs launched by Daily Picks: 0")
        st.write("• PRA session-state writes by Daily Picks: 0")
        st.write("• Points/Rebounds reads: 0")
        st.write("• Top-5 ranking: OFF")

    st.caption(
        "⚡ WNBA Daily Picks V2 Step 2 • PRA read-only state inspection • Step 1 shell preserved • Points/Rebounds paused • no ranking yet"
    )
