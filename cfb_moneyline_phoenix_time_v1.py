"""Display-only Phoenix kickoff conversion for CFB Moneyline.

Converts the verified CFB slate's Eastern calendar date + formatted Eastern
kickoff clock into Phoenix, Arizona local time with IANA zoneinfo. This module
owns no schedule fetch, projection, probability, market, edge, calibration,
or model logic.
"""
...
def phoenix_kickoff(game_date: Any, kickoff_et: Any) -> dict[str, str] | None:
    """Return Phoenix display labels from a verified ET date + kickoff clock."""
    day = _safe(game_date)
    clock_et = _safe(kickoff_et)
